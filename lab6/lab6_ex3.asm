...
UPPERCASE
	LDR R2, R1, #0			;r1 points to the letter in r0
	LD R3, CHANGECASE			;ascii number to check if upper or lower case
	ADD R2, R2, R3			;if number is negative, it means it is upper already since -97 add watever number below is going to be negative
	BRzp CHANGE_UPPER		;if number is positive or 0, means there is a lowercase letter since higher ascii number are lowercase number
	BRn ALREADY_UPPER
	
CHANGE_UPPER
	LDR R2, R1, #0			;r1 points to the letter in address r0
	LD R3, CHANGECASE2		;r2 gets -32
	ADD R2, R2, R3			;changing r1 to uppercase by subtracting 32 which will give you lowercase letter
	STR R2, R1, #0			;store the upper case letter back into the address
	ADD R1, R1, #1			;increment the address by 1
	ADD R5, R5, #-1			;decrement the counter of the letter
	BRp UPPERCASE				;if the counter is positive still, go back to first loop to go to the next letter
	BRz RESET					;if zero, go to test loop where it will reset r0 and r5
	
ALREADY_UPPER
	ADD R1, R1, #1			;increment address to check the next letter
	ADD R5, R5, #-1			;decrement counter by 1
	BRp UPPERCASE				;if positive, go back to first loop to loop through the next letter
	BRz RESET					;if zero, reset r0, r5
	
RESET
	LD R1, BACKUP_R1_3400	;resetting r0 back to x4000
	LD R5, BACKUP_R5_3400	;resetting r5 back to number of letters in word
